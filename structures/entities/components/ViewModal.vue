...
const populateFields = () => {
  ...
  fields.value = [
    ...
    { label: 'Test', value: item.value.test },
    
    { label: 'Name', value: item.value.name },
    
    { label: 'Email', value: item.value.email },
    
    { label: 'Age', value: item.value.age },
    
    { label: 'Active', value: item.value.active },
...
const fetchData = () => {
  if (props.entity) {
    item.value = props.entity
    populateFields()
  }
}

watch(isOpen, (newVal) => {
  if (newVal && props.entity) {
    fetchData()
  } else {
    item.value = null
    fields.value = []
